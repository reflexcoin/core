...
{
    // return if URI is not valid or is no reflex: URI
    if(!uri.isValid() || uri.scheme() != QString("reflex"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::RFLX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert reflex:// to reflex:
    //
    //    Cannot handle this later, because reflex:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("reflex://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "reflex:");
    }
...
{
    QString ret = QString("reflex:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::RFLX, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open reflex.conf with the associated application */
...
boost::filesystem::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Reflex.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Reflex (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Reflex (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "Reflex*.lnk"
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Reflex\n";
        else
            optionFile << strprintf("Name=Reflex (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Reflex app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Reflex app to startup item list
